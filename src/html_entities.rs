//! Go's HTML5 entity tables, for the `unescape` module.
//!
//! Generated by `tools/genhtmlent`. Do not edit by hand.
//!
//! # Provenance, and why this is a second entity table
//!
//! These are extracted from Go's standard-library `html` package
//! (`$GOROOT/src/html/entity.go`, BSD-3-Clause, The Go Authors), not from
//! blackfriday. Upstream's `escLink` calls `html.UnescapeString`, so matching
//! its behaviour means matching *Go's* table -- a different thing from the one
//! in `crate::entities`:
//!
//! | | `crate::entities` | this module |
//! |---|---|---|
//! | Source | blackfriday's `entities.go` | Go stdlib `html/entity.go` |
//! | Keys | include the leading `&` | do not |
//! | Values | `bool`, membership only | the decoded code point(s) |
//! | Entries | 2,231 | 2,138 plus 91 two-code-point |
//!
//! The 91 two-code-point entries are why Go keeps `entity2` separate:
//! `NotEqualTilde;` decodes to U+2242 followed by U+0338, which a `char`-valued
//! map cannot express.
//!
//! Both tables are byte-sorted so a binary search agrees with Go's map lookup.

/// Longest entity name that omits the trailing semicolon.
///
/// Go asserts this as a constant with a comment; a test below re-derives it
/// from the table so a regeneration cannot silently invalidate the
/// longest-match fallback that depends on it.
pub(crate) const LONGEST_ENTITY_WITHOUT_SEMICOLON: usize = 6;

/// Entity names to their single code point. Keys carry the trailing `;` when
/// the entity has one, and never the leading `&`.
static ENTITY: [(&str, char); 2138] = [
    ("AElig", '\u{00C6}'),
    ("AElig;", '\u{00C6}'),
    ("AMP", '\u{0026}'),
    ("AMP;", '\u{0026}'),
...
/// Entity names that decode to two code points.
static ENTITY2: [(&str, (char, char)); 91] = [
...
/// Looks up a single-code-point entity, mirroring Go's `entity[name]`.
pub(crate) fn entity(name: &[u8]) -> Option<char> {
    ENTITY
...
/// Looks up a two-code-point entity, mirroring Go's `entity2[name]`.
pub(crate) fn entity2(name: &[u8]) -> Option<(char, char)> {
    ENTITY2
        .binary_search_by(|(k, _)| k.as_bytes().cmp(name))
        .ok()
        .map(|i| ENTITY2[i].1)
...
mod tests {
    ...
    fn tables_are_sorted_and_unique() {
        for w in ENTITY.windows(2) {
            assert!(w[0].0 < w[1].0, "{:?} then {:?}", w[0].0, w[1].0);
        }
        for w in ENTITY2.windows(2) {
            assert!(w[0].0 < w[1].0, "{:?} then {:?}", w[0].0, w[1].0);
        }
    }
    ...
    #[test]
    fn counts_match_go() {
        assert_eq!(ENTITY.len(), 2138);
        assert_eq!(ENTITY2.len(), 91);
    }

    #[test]
    fn longest_without_semicolon_is_re_derived_from_the_table() {
        let bare: Vec<&str> = ENTITY
            .iter()
            .map(|(k, _)| *k)
            .filter(|k| !k.ends_with(';'))
            .collect();
        assert_eq!(bare.len(), 106);
        assert_eq!(
            bare.iter().map(|k| k.len()).max().unwrap(),
            LONGEST_ENTITY_WITHOUT_SEMICOLON
        );
        assert!(ENTITY2.iter().all(|(k, _)| k.ends_with(';')));
    }

    #[test]
    fn keys_have_no_leading_ampersand() {
        for (k, _) in ENTITY.iter() {
    ...
    #[test]
    fn spot_checks_against_go() {
        assert_eq!(entity(b"amp;"), Some('&'));
        assert_eq!(entity(b"amp"), Some('&'));
        assert_eq!(entity(b"lt;"), Some('<'));
        assert_eq!(entity(b"aacute;"), Some('\u{E1}'));
        assert_eq!(entity(b"nbsp;"), Some('\u{A0}'));
        assert_eq!(entity(b"notarealentity;"), None);
        assert_eq!(entity2(b"NotEqualTilde;"), Some(('\u{2242}', '\u{338}')));
        assert_eq!(entity2(b"amp;"), None);
    }
}
